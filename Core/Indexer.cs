...
public sealed class Indexer
{
    ...
    public Indexer(ILogger logger, int bufferSize = 8192)
    {
        _logger = logger;
    ...
    public Index? CreateIndex(string baseDirectory, bool calculateHashes = false)
    {
        ...
        try
        {
            ...
            // Process files
            var files = Directory.EnumerateFiles(basePath, "*", SearchOption.AllDirectories).ToList();
    ...
    private void CalculateFileHashes(string filePath, ConcurrentDictionary<string, IndexFile> fileData)
    {
        try
        {
            var relativePath = GetRelativePath(fileData);
    ...
    private static string? GetRelativePath(ConcurrentDictionary<string, IndexFile> fileData)
    {
        // Helper method to get relative path from dictionary
        return fileData.Keys.FirstOrDefault();
    }
